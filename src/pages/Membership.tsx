...
export default function Membership() {
  ...
  const printMemberForm = () => {
    ...
    const html = `
      <html>
        <head>
          <title>Membership Application - ${formData.name}</title>
          <style>
        ...
        <body>
          ...
          <div class="row">1. Membership No: <span class="val">${successId || ''}</span></div>
          <div class="row">2. Membership Fee: <span class="val">Rs. 100/-</span></div>
          <div class="row">3. Donation Rs: <span class="val">${formData.donation || ''}</span></div>
          <div class="row">
            <span class="item">4. Service No: <span class="val">${formData.serviceNo || ''}</span></span>
            <span class="item">Rank: <span class="val">${formData.rank || ''}</span></span>
            <span class="item">Name: <span class="val">${formData.name || ''}</span></span>
          </div>
          <div class="row">5. Name of Spouse / Next of kin: <span class="val">${formData.spouseName || ''}</span></div>
          <div class="row">6. Wing of the Defence Services: <span class="val">${formData.defenceService || ''}</span></div>
          <div class="row">
            <span class="item">7. A) Arm/Service: <span class="val">${formData.armService || ''}</span></span>
            <span class="item">B) Branch: <span class="val">${formData.branch || ''}</span></span>
          </div>
          <div class="row indent">C) Decorations (if any): <span class="val">${formData.decorations || ''}</span></div>
          <div class="row">
            <span class="item">8. A) Date of Retirement/Release: <span class="val">${formData.dateOfRetirement || ''}</span></span>
            <span class="item">Total Service: <span class="val">${formData.serviceYears || ''}</span></span>
          </div>
          <div class="row indent">(As per PPO / Discharge Certificate)</div>
          <div class="row indent">B) Date of Birth: <span class="val">${formData.dob || ''}</span></div>
          <div class="row">9. Amount of Basic Pension Rs <span class="val">${formData.basicPension || ''}</span> pm as per PPO</div>
          <div class="row indent">PPO No: <span class="val">${formData.ppoNo || ''}</span></div>
          <div class="row">10. Aadhaar Card No: <span class="val">${formData.aadharNo || ''}</span></div>
          <div class="row">11. PAN Card No (if issued): <span class="val">${formData.panNo || ''}</span></div>
          <div class="row">12. A) Permanent Address: <span class="val">${formData.permanentAddress || ''}</span></div>
          <div class="row indent">B) Current/Correspondence Address: <span class="val">${formData.address || ''}</span></div>
          <div class="row indent">
            <span class="item">C) Land Line No. with STD Code <span class="val">${formData.landline || ''}</span></span>
            <span class="item">Mobile No: <span class="val">${formData.phone || ''}</span></span>
          </div>
          <div class="row indent">D) Email ID: <span class="val">${formData.email || ''}</span></div>
          <div class="row">13. Occupation (Post Retirement), if any <span class="val">${formData.occupation || ''}</span></div>
          <div class="row indent">(Designation and Name of the organisation in which employed and its address)</div>
          <div class="row">14. Any other info you want to share: <span class="val">${formData.otherInfo || ''}</span></div>
