...
export default function Status() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const docRef = doc(db, 'members', appId.trim());
      const docSnap = await getDoc(docRef);

      if (docSnap.exists()) {
        setResult({ id: docSnap.id, ...docSnap.data() });
      } else {
        setError('No application found with that ID. Please check and try again.');
      }
    } catch (err) {
      ...
      <div className="max-w-xl mx-auto">
        <div className="text-center mb-10">
          ...
          <p className="mt-4 text-lg text-slate-600">
            Enter your Application ID to check your membership approval status.
          </p>
        </div>
